...
```{r q01}
#Fixed effect linear regression model. DV is riskSR. No random effect because only one measurement per participant.
```

Q2. First we will centre and scale (just scale) the age variable in the `self` dataframe and call it `age_cs` Note: this is what they did in the original study (unlike previous times when we have merely centrered it).
```{r q02}
...
#Separate plots
ggplot(self, aes(x=age, y=riskSR))+geom_point()+
  geom_smooth(method="lm") + facet_wrap(~sex)

#In one plot
ggplot(self, aes(x=age, y=riskSR, colour=factor(sex)))+geom_point()+
...
```{r q04}
ggplot(data = self, aes(x = age, y = riskSR, color = sex)) +
        geom_point() +
        geom_smooth(method = "lm") + 
  facet_wrap(~sex) +
        labs(title = "Self-rated Risk Taking across Age",
             x = "Age",
             y = "Self-reported Risk Taking")
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#The negative slope is steeper for female than males; females that are older rate themselves as less willing to take risks, where this association is less significant in males.
```
...
#Only one observation per participant, so no random effect (only applies when there are multiple outcomes)
```
...
#Yes, mod2 has bigger F-statistic and smaller p-value.
```
...
```{r q09}
gambles %>% group_by(scenario) %>% summarise(mean=mean(RiskyChoice)) 

#From normal conditions (gfk1-4), people mdade most riskier choice in gfk4.
#From high risk conditions (gfk5-8), people mdade most riskier choice in gfk5.
```
...
#People are less likely to make a risky choice but more likely to make a safe choice in investment when the trials are more risky ((e.g., low chance but high payoff). When trials are safer, they are more willing to take risks.
```
...
```{r q11}
# Scenarios would be random effects.
```
...
```{r q12}
mod4<-glmer(RiskyChoice ~ trialtype + (1 + trialtype | partid)
+ (1 | scenario), family="binomial", data = combi)
summary(mod4)
```
